DROP FUNCTION IF EXISTS canAdminTest;
DELIMITER $$
CREATE FUNCTION canAdminTest(in_user_id INT(11), in_facility_id INT(11)) RETURNS BOOLEAN
  DETERMINISTIC
BEGIN
  DECLARE tf BOOLEAN;

  SELECT IF(org_permission_group.name = 'Super Users',1,admin_edit_tests)
  FROM facility
