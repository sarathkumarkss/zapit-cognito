DROP FUNCTION IF EXISTS canAdminMachineType;
DELIMITER $$
CREATE FUNCTION canAdminMachineType(in_user_id INT(11), in_facility_id INT(11)
...
BEGIN
  ...
  SELECT IF(org_permission_group.name = 'Super Users',1,admin_manage_machines)
  FROM facility
  ...
  WHERE facility_user.user_id = in_user_id AND facility.id = in_facility_id
