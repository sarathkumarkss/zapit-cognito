SELECT
  ...
  -- America/New_York
  IFNULL(
    IF ( CONVERT_TZ( DATE(ADDDATE(test_run.max_test_date,machine_test_setup.frequency)), 'America/New_York', 'UTC') > FROM_UNIXTIME(:now), 0, 1),
...
LEFT JOIN (
  SELECT
    ...
    MAX(CONVERT_TZ(test_date,'UTC','America/New_York')) AS max_test_date,
    DATEDIFF(FROM_UNIXTIME(:now),MAX(test_date)) AS days_since_last_run
    -- DATEDIFF(CONVERT_TZ(NOW(),'UTC','America/New_York'),MAX(test_date)) AS days_since_last_run
  FROM test_run tr
